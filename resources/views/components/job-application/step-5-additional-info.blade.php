...
<div id="step-5" class="step {{ session('current_step', 1) == 5 ? '' : 'hidden' }}">
    ...
    <!-- Skills -->
    <div class="mb-4">
        <x-form-input label="Key Skills (comma-separated)" name="skills" id="skills" type="text" :value="old('skills', $formData['skills'] ?? '')" placeholder="e.g., JavaScript, Python, Project Management" required />
    </div>
    <!-- Certifications -->
    <div id="certificationContainer" class="mb-4">
        ...
        @foreach ($certifications as $index => $certification)
            <div class="certification-entry border p-4 rounded-md mb-2">
                <h3 class="text-lg font-medium text-gray-700">Certification {{ $index + 1 }}</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <x-form-input label="Resume (PDF/DOC)" name="resume" id="resume" type="file" accept=".pdf,.doc,.docx" required />
        ...
        <x-form-input label="Photo (Optional)" name="photo" id="photo" type="file" accept="image/*" />
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <div class="mt-1 flex space-x-4">
                <label><input type="radio" name="relocate" value="yes" {{ old('relocate', $formData['relocate'] ?? '') == 'yes' ? 'checked' : '' }} required> Yes</label>
        ...
        <div>
            ...
            <div class="mt-1 flex space-x-4">
                <label><input type="radio" name="eligible" value="yes" {{ old('eligible', $formData['eligible'] ?? '') == 'yes' ? 'checked' : '' }} required> Yes</label>
        ...
        <x-form-input label="Notice Period (days)" name="notice_period" id="notice_period" type="number" :value="old('notice_period', $formData['notice_period'] ?? '')" required />
    </div>
    <!-- References -->
    <div id="referenceContainer" class="mb-4">
        ...
        @foreach ($references as $index => $reference)
            <div class="reference-entry border p-4 rounded-md mb-2">
                <h3 class="text-lg font-medium text-gray-700">Reference {{ $index + 1 }} (Optional)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form-input label="Name" name="references[{{ $index }}][name]" :value="old('references.'.$index.'.name', $reference['name'] ?? '')" />
                    <x-form-input label="Contact" name="references[{{ $index }}][contact]" :value="old('references.'.$index.'.contact', $reference['contact'] ?? '')" />
                    <x-form-input label="Relationship" name="references[{{ $index }}][relationship]" :value="old('references.'.$index.'.relationship', $reference['relationship'] ?? '')" />
                </div>
            </div>
        @endforeach
    </div>
    <button type="button" id="addReference" class="text-blue-600 hover:underline mb-4">+ Add Another Reference</button>
    <div>
    ...
    <div class="mt-4 space-y-4">
        <label class="flex items-center">
            <input type="checkbox" name="declaration" value="1" {{ old('declaration', $formData['declaration'] ?? '') ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 @error('declaration') border-red-500 @enderror" required>
        ...
        @enderror
        <label class="flex items-center">
            <input type="checkbox" name="consent" value="1" {{ old('consent', $formData['consent'] ?? '') ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 @error('consent') border-red-500 @enderror" required>
